...
public class custInfo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            List loanapps=null;
            loanapps=session.createQuery("From CustMaster").list();
...
"</script>");
           ...
           out.println("<table class='table table-hover'>");
            ...
            out.println("</tr>");
            out.println("<form action='edit2.jsp'>");
            for (Iterator iterator = loanapps.iterator(); iterator.hasNext();){
                CustMaster loanapp=(CustMaster) iterator.next();
                out.println("<tr>");
                out.println("<td>"+loanapp.getCID()+"</td>");
                out.println("<td>"+loanapp.getCustName()+"</td>");
                out.println("<td>"+loanapp.getPANNo()+"</td>");
                out.println("<td>"+loanapp.getGender()+"</td>");
                out.println("<td>"+loanapp.getCity()+"</td>");
                out.println("<td>"+loanapp.getAddress()+"</td>");
                out.println("<td>"+loanapp.getMobileNo()+"</td>");
                out.println("<td>"+loanapp.getAlternateNo()+"</td>");
                out.println("<td>"+loanapp.getDob()+"</td>");
                out.println("<td>"+loanapp.getStatus()+"</td>");
                out.println("<td>"+"<input type=\"checkbox\" name=\"names\" value=\""+ loanapp.getCID()+ "\"  onclick='onlyOne(this)'/>"+"</td>");
                out.println("</tr>");
            }
           out.println("</table>");
           out.println("<input type='submit' value='Edit'>");
           out.println("<input type='submit' value='Delete' formaction='del2'>");
           out.println("</form>");
           out.println("</div>");
           out.println("</center>");
        }
